...
export async function POST(req: NextRequest) {
  ...
  const userInDb = await prisma.user.findUnique({ where: { id: body.id } });
  ...
  if (!userInDb) {
    ...
    return NextResponse.json({ status: 'success', message: 'User created', data: newUser }, { status: 201 });
  }

  const hasUserChanged = [
    userInDb.name !== body.userName,
    userInDb.fisrtName !== body.firstName,
    userInDb.lastName !== body.lastName,
    userInDb.hash !== body.hash,
  ].some(Boolean);
  ...
  if (hasUserChanged) {
    ...
    return NextResponse.json({ status: 'success', message: 'User updated', data: updatedUser }, { status: 200 });
  }

  return NextResponse.json({ status: 'success', message: 'Get user', data: userInDb }, { status: 200 });
}
